...
export default function Chart({ className = '' }: ChartProps) {
  ...
  const statusData = [
    {
      name: 'Completadas',
      value: tasks.filter((task: Task) => task.status === 'done').length,
      color: '#10b981'
    },
    {
      name: 'Pendientes',
      value: tasks.filter((task: Task) => task.status === 'pending').length,
      color: '#f59e0b'
    }
  ]
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold">
              {tasks.length > 0 ? Math.round((tasks.filter((t: Task) => t.status === 'done').length / tasks.length) * 100) : 0}%
            </div>
            <div className="text-sm opacity-90">Tasa de Completitud</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold">
              {tasks.filter((t: Task) => t.priority === 'high').length}
            </div>
